	...
	protected function link_to ()
	{
		...
		switch ( gettype($_options) ):
			...
			case 'array':
				...
				unset($_parts['controller'], $_parts['action'], $_parts['id'], $_parts[0], $_parts['confirm'], $_parts['fragment']);
		...
		$_link = "<a href='{$server}{$base_url}/{$_url}{$_query}{$_fragment}' title='{$_text}'{$_confirm}>{$_text}</a>";
